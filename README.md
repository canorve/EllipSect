...
ElllipSect.py creates surface brightness profiles
...
```
 ./ElllipSect.py [GALFITOutputFile] [-logx] [-q AxisRatio] [-pa PositionAngle] [-comp] [-pix] [-ranx/y Value] [-grid] [-dpi Value] [-noplot] [-phot] [-sbout] [-noplot] [-minlevel Value] [-sectors Value] [-object Name] [-filter Name] [-snr] [-help] [-checkimg] [-noned] [-distmod Value] [-magcor Value] [-scalekpc Value][-sbdim Value] [-model ModelImage] [-keep] [-ned XmlFile] 
 ```
...
* ElllipSect works for GALFIT version $> 3.0.7$ Check this because you will
    have issues for *-comp* option.

* ElllipSect uses the mask image (option "*F*" GALFIT) only if this
    is a **FITS** image. In case your mask is an *ASCII* file, you can convert it to **FITS** using the [xy2fits.py](xy2fits.md) tool.


* ElllipSect uses axis ratio (*q*) and position angle (*pa*) to create an "ellipse" *grid* using the function *sectors_photometry* from the *mgefit* library. Unlike IRAF's Ellipse, *q* and *pa* are fixed through radius. See the two images below: 

    ![A85 gal](img/A85.img.png)
    ![A85 mod](img/A85.img.mod.png)

    For this reason, errors are expected to 
    be greater than those coming from IRAF's ellipse since ElllipSect 
...
* Since ElllipSect reads the "*B*" option of galfit.XX file, this
...
* In order for the program to detect the components, they must share the same 
    center (x,y). This allows that galfit can use other components such as
     sersic to be used as masks for nearby GALFIT. ElllipSect already does not 
...
* To manually introduce an axis ratio of 0.35 and position angular of 60 
    (measured from Y-axis): 
     ...
     ./ElllipSect.py galfit.02 -q 0.35 -pa 60 
...
* Same as above but popup window does not appear. Plots are 
    ...
    ./ElllipSect.py galfit.46 -grid -dpi 300 -noplot 
...
* If the user desires to create their own plots, 'sbout' option
  ...
  in a file:
    ...
    ./ElllipSect.py galfit.46 -sbout

  ElllipSect can also save the surface brightness data for 
  individual components in separated files:

    ./ElllipSect.py galfit.46 -comp -sbout 

### Phot Examples

*   *ElllipSect* can calculate additional info besides the ones 
...
*   *phot* option looks in NED (NASA/IPAC Extragalactic Database) for 
    ...
    ElllipSect looks for name of the galaxy (as it is searched in NED) 
...
*   ElllipSect allows to introduce manually the NED info. For example, 
    ...
    ./ElllipSect.py galfit.10 -phot -distmod 10 -magcor 0.3 -scalekpc 1.3 -sbdim .3

    This option avoids to connect with NED. 

    ElllipSect does not correct by K-correction. 


### Advanced Examples

*   model option allows the user to introduce his own model image for analysis.
    ElllipSect will use this image instead of the one created by GALFIT output.
    If -phot option is enabled, SNR quantities will be inaccurate.

    ./ElllipSect.py galfit.14 -model model.fits
...
*   sky option allows the user to introduce his/her own sky value to substract 
    ...
    otherwise ElllipSect will produce wrong outputs.  

    ./ElllipSect.py galfit.01 -sky 300
...
*   The following options requires that the user has already experienced with 
    ...
    ./ElllipSect.py galfit.14 -minlevel 0
...
*   sectors option is another parameter that is given directly to *sectors_photometry*.
    ...
    ./ElllipSect.py galfit.14 -sectors 19
...
*   checkimg will create images used by *sectors_photometry* to check how 
    ...
    ./ElllipSect.py galfit.14 -comp -checkimg




**ElllipSect.py** uses the mgefit library which is
